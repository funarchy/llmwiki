...
describe('syncDeps', () => {
  it('happy path: vendors an npm dep, rewrites links, writes index and a matching lock', () => {
    ...
    const indexContent = readFileSync(join(repo, 'wiki', 'deps', 'a', 'index.md'), 'utf-8');
    expect(indexContent).toContain('/wiki/deps/a/topic.md');
    expect(existsSync(join(repo, 'wiki', 'deps', 'index.md'))).toBe(true);
  ...
  it('hoists a transitive npm dep flat, locking requiredBy and resolvedFrom for a nested-only producer', () => {
    ...
    expect(existsSync(join(repo, 'wiki', 'deps', 'b', 'index.md'))).toBe(true);
  ...
  it('is idempotent: running twice produces byte-identical deps/ tree and lock', () => {
    ...
    const depsDir = join(repo, 'wiki', 'deps');
  ...
  it('removes a vendored bundle whose dep was dropped from config', () => {
    ...
    expect(existsSync(join(repo, 'wiki', 'deps', 'b'))).toBe(true);
  ...
  it('produces a tree that passes every registered lint check end to end', () => {
    const repo = makeRepo({
      // skills: off — this test is about dependency-vendoring conformance,
      // not skills sync state; check 12 would otherwise warn for five
      // unlocked shipped skills this fixture never installed.
      'wiki-sticky.yaml': 'version: 1\nbundle:\n  root: wiki\nskills: off\ndeps:\n  a: npm\n',
