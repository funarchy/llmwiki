...
consumer root `wiki-sticky`, bundle `@scope/pkg`):

| Producer's link | Becomes |
| --- | --- |
| `/wiki/pms/traits.md` (producer's own page) | `/wiki/deps/@scope/pkg/pms/traits.md` |
| `/wiki/deps/@scope/other/x.md` (already-hoisted transitive) | `/wiki/deps/@scope/other/x.md` — flat, not nested |
...
conventionally use `wiki/` — the first case is a no-op and only the
