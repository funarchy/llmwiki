...
`wiki-sticky install` (via `add`, `rm`, `update`) and is read-only by
...
`wiki-sticky.yaml`'s `vendor:` block (`from:` a URL or path) — `install` never
fetches it automatically.

This repository declares no dependencies and synthesizes nothing, so it has
no `deps/` or `vendor/` directories at all; `wiki-sticky lint`'s checks 9–11,
