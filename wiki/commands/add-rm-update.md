...
description: How the three dependency-editing commands change wiki-sticky.yaml and re-sync
tags: [commands]
sources:
  - src/commands/add.ts
  - src/commands/rm.ts
...
`add`, `rm` and `update` all edit `wiki-sticky.yaml` (or nothing, for `update`)
...
`wiki-sticky.yaml` — it probes `resolveGraph` against a config copy that already
...
**`update [pkg]`** re-resolves without editing `wiki-sticky.yaml` at all, then
...
`/wiki/deps/index.md` or `/wiki/vendor/index.md` from the root index
