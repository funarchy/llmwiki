...
`llmwiki init` calls `syncSkills` against the just-written config,
installing the five shipped skills into `.claude/skills/` and
`.agents/skills/`; `llmwiki skills sync` does the same standalone.
Currency is hash-locking: the installed copies are recorded in
`llmwiki-lock.json`'s `skills` section and checked for drift by lint
