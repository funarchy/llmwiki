...
describe('templates', () => {
  ...
  it('pre-commit.sh guards against wiki-sticky not being installed, with a from-checkout fallback and no network calls', () => {
    const content = readFileSync(join(packageRoot(), 'templates', 'pre-commit.sh'), 'utf-8');
    expect(content).toContain('node_modules/.bin/wiki-sticky');
    // The dogfood fallback: a built checkout of wiki-sticky itself...
    expect(content).toContain('dist/cli.js');
    // ...and only wiki-sticky itself — never a consumer's own dist/cli.js.
    expect(content).toContain('"name": "wiki-sticky"');
