...
llmwiki ships five model-invoked skills, canonically `skills/wiki-*/SKILL.md`
in this repository, installed into a consumer's `.claude/skills/` and
`.agents/skills/` by `llmwiki init` or `llmwiki skills sync`
...
- **`wiki-review`** — adversarial, advisory-only. Triggers after a
  ...
  `vendor/**`. Mechanical link/format problems are `llmwiki lint`'s job,
  not this skill's.
...
`llmwiki-lock.json`'s `skills` section and checked for drift by lint check
